...
ICM20948_WE imu = ICM20948_WE(CONFIG_I2C_ADDR);
...
void getMagData(double *magX, double *magY, double *magZ) {
	...
	*magX = (double)(magValue.x + CONFIG_MAG_OFFSET_X);
	*magY = (double)(magValue.y + CONFIG_MAG_OFFSET_Y);
	*magZ = (double)(magValue.z + CONFIG_MAG_OFFSET_Z);
}
...
void getMotion6(double *_ax, double *_ay, double *_az, double *_gx, double *_gy, double *_gz) {
	...
	imu.readSensor();
	imu.getGValues(&aVal);
	imu.getGyrValues(&gVal);
	//printf("%f %f %f %f %f %f\n", aVal.x, aVal.y, aVal.z, gVal.x, gVal.y, gVal.z);
